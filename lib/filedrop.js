...
Filedrop = (function(_super) {
  ...
  Filedrop.prototype._init = function() {
    ...
    this.dropzone = $(this._dropzoneTpl).find('.filedrop-hints').html(this.opts.hints).end().hide().appendTo(document.body);
    $(document).on("dragover.filedrop", (function(_this) {
      return function(e) {
    ...
    })(this)).on('drop.filedrop', function(e) {
      return e.preventDefault();
    }).on('dragenter.filedrop', (function(_this) {
      return function(e) {
        if ((_this._entered += 1) === 1) {
          _this.showDropzone();
          return _this.trigger("fileDropShown", e);
        }
      };
    })(this)).on('dragleave.filedrop', (function(_this) {
      return function(e) {
        if ((_this._entered -= 1) <= 0) {
          _this.hideDropzone();
          return _this.trigger("fileDropHidden", e);
        }
      };
    })(this));
    return this.dropzone.on("dragover", (function(_this) {
      ...
      return function(e) {
        ...
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          file = _ref[_i];
          if (!(_this.opts.types.indexOf(file.type) > -1)) {
            _this.trigger("fileDropfail", [file, "Wrong types!"]);
            _this.hideDropzone();
            return false;
          }
          files.push(file);
  ...
  Filedrop.prototype.showDropzone = function() {
    var elOffset;
    return this.dropzone.removeClass('hover').css({
      zIndex: 9,
      position: 'absolute',
